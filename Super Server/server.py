...
class Server:
    ...
    def sender(self, content, conns):
        try:
            ...
            print('send message fail. try again')
    ...
    def check_signup(self, username,pw):
        sign = dbms.checkSignup(username,pw)
        packet = {}
        packet["response"] = "signup"
        packet["host"] = (SERVER["IP"], SERVER["PORT"])
        packet["data"] = {}
        if(sign):
            packet["data"]["state"]="Failed"
        else:
            packet["data"]["state"]="Success"
        packet = json.dumps(packet)
        return packet
        ...
        try:
            while connected:
                ...
                if "file" in packet:
                    ...
                    elif packet["request"] == "signup":
                        
                        username = packet["data"]["username"]
                        pw = packet["data"]["password"]
                        packet = self.check_signup(username,pw)
                        self.sender(packet, [conn])
            ...
            dbms.resetRoomIp(dbms.selectUserRooms(username))
            conn.close()
